...
public class IntegerJsonValueGeneratorTests
{
    [SetUp]
    public void Setup()
    {
    ...
    private static IEnumerable<TestCaseData> _generateValueCaseData = new[]
    {
        ...
        }).SetName("WithAllMinimumMaximumSchemaProperties"),
        new TestCaseData(new Dictionary<string, object> {
            {"multipleOf", 2}
        }).SetName("WithMultipleOfSchemaProperties"),
        new TestCaseData(new Dictionary<string, object> {
            {"multipleOf", 7},
            {"minimum", 0},
            {"maximum", 100}
        }).SetName("WithMultipleOfMinimumMaximumSchemaProperties"),
        new TestCaseData(new Dictionary<string, object> {
            {"multipleOf", 8},
            {"minimum", 0},
            {"maximum", 100},
        ...
        (Dictionary<string, object>? schemaParameters)
    {
        ...
        var generator = new IntegerJsonValueGenerator(new Random().Next());
        ...
        // Assert
        var value = generatedJsonValue.GetValue<int>();
        if (exclusiveMinimum) Assert.That(value > minimum, Is.True);
        Assert.That(value >= minimum, Is.True);
        if (exclusiveMaximum) Assert.IsTrue(value < maximum);
        Assert.That(value <= maximum, Is.True);
        if (multipleOf is not null) Assert.That(value % multipleOf, Is.EqualTo(0));
    }
}